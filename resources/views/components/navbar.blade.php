...
<nav class="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-neutral-100" x-data="{ mobileOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <div class="hidden md:flex items-center gap-8">
                ...
                @if(session('dev_role') === 'customer')
                <a href="/pesanan/SR12-2024-0582" class="text-sm font-medium {{ request()->is('pesanan*') ? 'text-primary-600' : 'text-neutral-600 hover:text-primary-600' }} transition-colors">Pesanan Saya</a>
                @endif
            </div>
            ...
            <div class="hidden md:flex items-center gap-3">
                ...
                <a href="/keranjang" class="relative p-2 text-neutral-600 hover:text-primary-600 transition-colors">
                    ...
                    <span class="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 bg-primary-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center">3</span>
                </a>
                {{-- Avatar --}}
                ...
                {{-- Logout (dev) --}}
                <a href="/dev/logout" class="p-2 text-neutral-400 hover:text-red-500 transition-colors" title="Logout (Dev)">
    ...
    <div x-show="mobileOpen" x-cloak
        ...
        <div class="px-4 py-4 space-y-3">
            ...
            @if(session('dev_role') === 'customer')
            <a href="/pesanan/SR12-2024-0582" class="block py-2 text-sm font-medium {{ request()->is('pesanan*') ? 'text-primary-600' : 'text-neutral-600' }}">Pesanan Saya</a>
            <a href="/keranjang" class="block py-2 text-sm font-medium text-neutral-600">Keranjang</a>
            <hr class="border-neutral-100">
            <a href="/dev/logout" class="block w-full text-center py-2.5 text-sm font-semibold text-red-500 border border-red-200 rounded-xl">Keluar</a>
            @else
            <hr class="border-neutral-100">
