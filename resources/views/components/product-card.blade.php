...
<div class="card group overflow-hidden">
    ...
    <div class="relative aspect-4/5 bg-neutral-100 overflow-hidden">
        ...
        @else
            <div class="w-full h-full flex items-center justify-center bg-linear-to-br from-neutral-100 to-neutral-200">
    ...
    <div class="p-4">
        ...
        {{-- Add to Cart Button --}}
        @auth
            <form action="/keranjang/tambah/{{ $product['slug'] }}" method="POST" class="mt-3">
                @csrf
                <button type="submit"
                   class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-primary-600 text-white text-xs font-semibold rounded-lg hover:bg-primary-700 transition-all duration-200 {{ $product['stock_status'] === 'Out-of-Stock' ? 'opacity-50 pointer-events-none' : '' }}">
        ...
        @else
            <a href="/login" class="mt-3 w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-primary-600 text-white text-xs font-semibold rounded-lg hover:bg-primary-700 transition-all duration-200">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
