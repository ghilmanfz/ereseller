...
<div class="card group overflow-hidden">
    ...
    <div class="relative aspect-[4/5] bg-neutral-100 overflow-hidden">
        @if($product['stock_status'] !== 'Out-of-Stock')
            @auth
                <form action="/keranjang/tambah/{{ $product['slug'] }}" method="POST" class="absolute inset-0 z-10">
                    @csrf
                    <button type="submit" class="w-full h-full" aria-label="Tambah {{ $product['name'] }} ke keranjang lewat gambar"></button>
                </form>
            @else
                <a
                    href="/login"
                    ...
                    aria-label="Login untuk tambah {{ $product['name'] }} ke keranjang"
                ></a>
            @endauth
        @endif
